...
async function GetAll(req) {
  ...
  let newData
  if(country !== null){
    // console.log("indide not null", country)
    if(priceRange === undefined){
      newData = await Model.find({"address.country":country[0].value},{"_id":true,password:0,verifyToken:0})
      .where("amenities" )
      .in([...amenities])
      // .sort(sortBy)
      .skip(page * limit)
      ...
      newData = await Model.find({"address.country":country[0].value ,$and: [
        { price: { $lt: maxPrice } },
        { price: { $gt: minPrice } },
    ...
    // console.log("indise country null")
    if(priceRange === undefined){
